...
角色资源账本开始进入长篇主链。系统会帮你追踪角色手里的道具、线索、身份凭证、底牌和代价，并把这些资源变化带回后续章节写作、审阅、修复和卷规划里，减少长篇连载中“前面给过的东西后面忘了”的断裂。
...
- 小说任务抽屉会集中显示资源变更待确认项，可以查看来源、确认入账或忽略变化。
- 章节执行页支持手动复查本章资源，角色准备页支持回填最近章节资源，旧项目也能轻量补齐角色背包。
- 卷战略页会显示本卷关键资源承诺，规划当前卷时更容易看清可用手段、铺垫边界和后续兑现点。

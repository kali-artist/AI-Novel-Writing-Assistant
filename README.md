...
拆书结果页更适合持续阅读和核对：任务操作固定在顶部工具栏，分析信息、发布和运行元信息默认折叠；小节 Tab 更早进入首屏，证据从独立长面板收进对应小节内，可以直接查看字段绑定证据和原文摘录。

- 顶部 sticky 工具栏承载复制、重新生成、任务中心、导出、生成写法和归档入口，滚动阅读时不用回到页面顶部找操作。

...
自动导演开书的方案确认更清楚：生成书级方向后，候选方案会在起始设置弹窗上方单独弹出，用户不用滚到表单下方寻找结果。

- 生成第一批方案、继续生成下一轮、微调单套方案或重做标题组后，页面会打开独立的“确认书级方案”弹窗，方便直接比较两套方向并选用。
- 起始设置弹窗保留“查看已生成方案”入口，关闭方案弹窗后也能重新打开结果，不必重新生成。
- 自动导演推进中，工作台会跟随当前步骤和进度刷新对应内容。AI 写入故事规划、角色、卷章、章节或质量结果后，不必等任务暂停或手动刷新页面才看到最新填充内容。
...
- 章节正文改动后，角色关键资源同步会按新的正文内容重新检查，后续章节更容易继续拿到最新的道具、线索、底牌和资源限制。
- 角色页可以为当前角色或全书角色补齐外貌、体态、常见穿着、标志细节、声音口吻和登场印象；确认后写入角色卡，已有明确内容会保留。
- 自动导演应用角色阵容后，会为新建或更新角色自动补齐缺失的稳定外显资料，低质量建议会跳过，不影响角色准备继续推进。
- 章节生成会带入参与角色的短外显摘要，优先提供样貌/体态、标志细节和声音口吻，让正文更容易写出读者记得住的角色。
- 角色外显资料生成后的反馈会直接显示在按钮下方，包含生成进度、可写入项和确认入口，不必滚到资料卡底部寻找结果。
- 角色外显资料补齐支持填写倾向，可以指定更有压迫感、带一点病弱感、声音更温和等方向，再生成当前角色或全书角色的可写入建议。
- 填写补全倾向时，当前角色可以生成覆盖已有内容的建议；页面会先展示差异和提醒，确认后再保存到角色卡。

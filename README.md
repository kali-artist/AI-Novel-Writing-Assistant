...
- 世界库和世界工作台会以世界卡片、世界手册、核心规则、主要势力、关键地点、关系网络和完整度诊断来展示结果，减少直接面对字段表单的负担。
- 世界地图和势力图谱拥有更完整的数据支撑，地点、方位、控制势力、冲突热度和势力关系可以用于更直观的图谱展示。
...
- 章节生成后的结构化审校、时间线抽取和资产同步也更稳，常见字段别名会被自动归一，减少因为 JSON 结构不匹配导致的修复重试或章节流程中断。

...
自动导演后台与桌面预发布构建进一步对齐：小说创作 API 服务启动时会同时拉起导演命令消费循环，一般不必再单独启动第二个后端进程处理排队任务；命令执行链也继续收束，减少旧双轨运行时带来的对齐成本。

- 与 RAG 等后台任务类似，导演命令队列随主服务同进程启动；本地开发或单机部署通常只需保持一套服务端进程运行。
- 导演后台命令仍统一走同一条执行链；运行投影、章节产物进度与继续恢复的判断方式与前几日更新说明一致，并继续优先参考当前工作区真实数据。
- 预发布桌面版仍通过既有发布渠道提供 Windows 安装包；关注仓库 Release 页面即可获取 Beta 构建。
- 自动导演显示态继续收口到“事实优先”首步判断：小说页、任务中心和任务抽屉会优先展示统一推导的当前步骤、阻塞原因和建议动作，减少跨入口状态错位。

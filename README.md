...
分场景写正文会重新服从整章目标字数：章节目标 3000 字时，系统会把场景预算、单轮上限和整章剩余额度一起交给写作链路，避免每个场景都扩写成一整章。

- 短场景和 `prompt_only` 模式也会带着硬上限生成；整章达到硬上限后，后续场景会停止生成。
- 章节审校会继续检查 `prompt_only` 章节的最终长度，超过硬上限会生成关键风险提醒。
- 自动执行遇到已标记“需要修复”的章节时会停在该章处理，不会因为正文状态曾经通过而继续跳到后续章节。
...
- 桌面客户端推进到 `0.3.8`，用于发布本次章节合同补强、空章节防线、候选重新生成修复和主角识别体验优化后的 Windows 客户端包。

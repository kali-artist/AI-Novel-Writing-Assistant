...
- 自动导演恢复时遇到并发变化会更稳，过期的产物依赖不会再轻易中断整条恢复链；恢复样本检查也会识别已经被同一小说更新任务取代的旧失败任务。
- 已有小说接管任务恢复时，会优先复用原接管请求里的上下文；任务还没写入完整导演输入时，也能继续从接管入口恢复。
- 已有小说接管后的继续、重试和恢复入口会统一检查原接管上下文，减少从“继续前 10 章”等入口进入后又提示缺少恢复上下文的情况。
- 继续前 10 章自动执行时，系统会按用户选择进入章节写作；已具备执行资源的旧任务不会误回到节奏 / 拆章重新推进，也不会在章节执行入口再次停回等待确认；资源不完整时仍会先补齐规划。
- 继续自动执行前 10 章时，章节写作后的自动审稿和状态同步会跟随同一次确认推进，减少在写作、审稿和同步节点之间反复确认。
...
- 历史正文的章节账本基线可以安全补齐。系统会先区分已追踪正文和真正缺少基线的历史章节，补齐时只新增缺失记录，不覆盖已有正文或已有账本。

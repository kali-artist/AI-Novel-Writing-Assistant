...
彻底修复了正文即兴写出的硬设定（交易性质、金额、票号、数量等）无法跨章保持一致的问题；同时短剧创作已经有可进入的工作台，可以从小说、原创灵感或导入文本进入短剧策略、分集、台本、质量检查、分镜和视频提示词流程。
...
- 新增“短剧工作台”入口和项目详情工作台，桌面侧栏和移动端创作辅助导航都可以进入；进入后可以选择已有小说、原创灵感或导入文本创建短剧项目，并在项目内查看素材、策略、分集、台本、质量检查、分镜视频提示词和导出入口。单集台本和角色卡支持直接编辑保存，角色库可以导入复用，素材页会提示梗概、节拍、角色和硬事实是否足够支撑后续生产。

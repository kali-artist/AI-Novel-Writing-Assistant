...
拆书新增预算上限、熔断保护、扩容续跑、角色候选识别、发布来源隔离和结构化检索：创建拆书时可以设置本次任务的 token 预算，生成过程中会累计用量；达到上限后任务停止并保留已完成小节。详情页可以调整预算，预算用尽失败后可扩容并续跑未完成小节。角色档案可以先低成本识别候选，再按需生成单个或全部深度档案。拆书发布到知识库时会标记为“拆书发布”，重复发布同一份拆书会追加为同一知识文档的新版本，避免和用户上传文档混合；发布版的题材、卖点、读者、优劣势和章节锚点会进入索引元数据，便于后续创作更精准召回。RAG 召回也会记录后端 trace，为后续召回回放和质量评估准备数据底座。

- 新建拆书和诊断拆书都会携带预算上限，默认值与后端运行配置保持一致。
- 预算用尽时，任务会以 `budget_exceeded` 标记失败，已生成成功的小节继续可读、可复制和可发布。
- 拆书详情页顶部和运行元信息会显示已用 tokens / 预算上限，帮助用户判断是否缩小范围、减少维度或重新设置预算。
- 详情页支持单独调整预算；预算用尽失败后可以扩容并续跑，系统只补未完成小节，成功小节和冻结小节保留。
- 拆书并发执行在出现错误后会停止派发新小节，并等待已启动的小节收尾后统一结束，减少失败状态被并发进度覆盖的风险。
- 拆书失败后重新生成会保留已经成功的小节，优先补完未完成内容，减少预算用尽后整单重做的成本。
- 角色档案支持先识别候选，再生成单个或全部深度档案；候选会显示定位、重要度、简述和出场章节提示。
- 角色识别和档案生成按需计入拆书 token 用量，但不会改变基础拆书任务状态，也不会随全量拆书自动触发。
- 知识库列表和资料选择器会区分“上传文档”和“拆书发布”，拆书发布文档可跳回来源拆书查看。
- 拆书发布版的结构化关键结论会写入 RAG 元数据；带章节定位的证据会在 chip 上标出“原文”，便于回到章节核对。
- 已完成索引的知识库文档可以直接从文档列表进入召回测试，方便核对这份资料在创作检索中的命中效果。
- RAG 召回会按采样率记录查询摘要、检索范围、候选数量、命中摘要、阶段耗时和 fallback 标记，便于后续诊断召回质量。
- 拆书结果页改为「小节分析 / 角色档案」顶层视图互斥切换，顶部工具栏跨视图共享、始终可见；视图状态写入 URL `?view=`，刷新和分享链接都会回到对应视图。

...
自动导演模式完成一轮整合升级：整本推进、章节执行、恢复链、AI 驾驶舱和任务入口被收成更清楚的一条主链。

- 全书自动推进遇到普通状态提案、可修复审校问题和质量建议时，会优先由 AI 自动继续、修复、整章重修或重规划；只有硬故障、连续失败和受保护正文冲突才停下来。
- 局部补丁失败会自动升级整章重修，并在重修后重新审校，减少“目标片段不存在”把整本书卡住的情况。
- 章节列表里的质量风险会显示为中文标签，不再把后台 JSON 原文露给用户。
- 自动导演进度会给出阻塞原因、推荐动作、AI 是否可自动恢复、拆分进度和风险标签，任务面板与工作台读取同一份状态。
- 后台 Worker 租约过期时，整本自动推进任务会优先从最近安全进度自动重新排队，连续失败超过阈值后才进入待恢复。
- 章节执行页和左侧流程会跟随后台真实节点显示进展，正文开始生成后不会继续停留在“节奏 / 拆章”。
- 章节修复完成后会跟随最新质量复检切回可继续状态，避免同一章已经可继续却仍显示“一键修复”。
- AI 已生成或已修复的章节不会被误判为用户手写受保护内容，继续自动导演时更少被旧覆盖风险打断。
- 历史修复工单会跟随质量复检结果自动失效，已处理章节不会继续占用待修复摘要。
- 全书自动推进恢复后会继续保持自动导演策略，普通状态提案和可修复审校问题不会把任务退回人工审核；审核阻断也不会再提前留下“生成中”的章节状态。
- 选择书级方向后，AI 可以连续准备故事规划、角色资产、卷战略、节奏拆章、章节任务单、正文生成、审校、修复和状态同步，帮助新手少判断“下一步去哪”。
- 每本小说都有自己的 AI 驾驶舱。小说列表、小说工作台和执行详情会展示同一份状态、主原因、最近进展、AI 用量、产物记录和唯一主动作。
- 任务入口更清楚。左侧只保留轻量状态入口，完整进度放进弹窗和执行详情；任务中心继续用于后台历史、筛选和排查。
- 等待确认时会直接露出“确认并继续”。遇到可能影响用户编辑或受保护内容的动作，用户不用在日志和任务中心里找确认按钮。
- AI 用量会按章节规划、正文写作、质量检查、文本修复、风格调整、伏笔同步、角色 / 状态同步等阶段拆开展示，减少把整套质量闭环误看成“正文写作”的困惑。
- 章节执行会减少重复成本。伏笔同步、角色资源抽取、状态快照和角色动态抽取会尽量只在最终保留正文确定后执行。
- 章节局部修复更稳。补丁失败时会尝试确定性的空白等价匹配；仍无法安全替换时，会升级为整章重修并复审。
- 章节任务单和章节质量闭环更严格。正文生成前会检查任务单是否可靠；写完后会结合轻审、必要修复、复审、留存、连续性和近期复盘判断是否继续、修复、重修或重规划。
- 重规划更贴近真实故事状态。AI 会结合章节目标、审校问题、伏笔账本和当前状态判断调整窗口，完成后接回后续章节批次。
- 全书自动推进增加暂停保护。重复失败、修复失败、异常用量或可能影响受保护正文时，系统会暂停新批次并给出恢复建议。
- 恢复链覆盖更多真实场景。服务重启、后台命令过期、旧批次失败、章节正文为空但状态显示完成、旧产物缺少账本等情况，会更准确地回到可继续位置。
- 小说列表也能打开单本书驾驶舱，快速查看这本书的 AI 运行状态、最近进展和可执行主动作。

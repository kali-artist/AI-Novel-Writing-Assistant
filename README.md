...
- 新增“短剧工作台”入口和项目详情工作台，桌面侧栏和移动端创作辅助导航都可以进入；进入后可以通过“来源 → 内容 → 规格”向导创建短剧项目，并在项目内查看素材、策略、分集、台本、质量检查、分镜视频提示词和导出入口。新建时可让 AI 推荐赛道，素材页也会生成补充问题，帮助先补齐梗概、节拍、角色和硬事实再继续生产。

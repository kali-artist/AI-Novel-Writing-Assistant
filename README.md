...
模型路由对不同渠道的兼容性控制更细了，自动导演的章节推进顺序也更稳了。现在可以按任务配置请求协议和结构化输出格式，也可以给模型供应商设置同模型并发上限和请求间隔；继续节奏拆章或旧自动导演任务时，系统会按真实章节资产重新确认目标范围内所有章节都已细化，并清理执行区里和最新拆章不一致的旧章节数据，再按单章写作和质量修复依次推进。分卷、章节细化和故事规划保存也更稳定，减少模型已返回但结果没落库导致的重复调用。

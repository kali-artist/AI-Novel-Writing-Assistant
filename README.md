...
彻底修复了正文即兴写出的硬设定（交易性质、金额、票号、数量等）无法跨章保持一致的问题；同时短剧创作链路推进到后端骨架，可以从小说、原创灵感或导入文本进入短剧策略、分集、台本、质量检查、分镜和视频提示词流程。
...
- 短剧创作新增项目、内容源、防腐层、台本、质量闸、修复、导出、角色库、分镜、视频提示词和视频 provider 抽象的后端能力，为后续 `/drama` 工作台打基础。

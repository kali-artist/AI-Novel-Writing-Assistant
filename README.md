...
- 创作中枢可以通过自动导演运行时读取状态、解释下一步、评估改文影响并请求继续推进；继续执行和高自动化策略会先进入确认流程。
- 提示词工作台可以查看已登记的基础提示词、可配置槽位边界和参与预览的上下文块；预览只展示最终发送内容，不保存自定义覆盖，也不会直接调用模型。
- 创作中枢资源绑定、近期对话、小说基础信息状态和整本生产状态会按标准上下文块进入提示词预览，方便后续确认哪些信息会影响 AI 判断。
- 章节写作、章节审校和自动导演工作区分析开始共用统一上下文组织方式，让提示词预览里看到的上下文块和真实执行更一致。
- 章节修复会带上更完整的角色动态提示，局部修文时更容易保留角色职责、目标和关系压力。
- 自动导演候选、书级规划、章节写作、质量检查、修复和状态同步会按统一步骤计划推进，章节流水线、手动按钮和创作中枢后续可复用同一套执行能力。
- 自动导演的运行、步骤、事件、产物和依赖关系开始写入独立账本，继续任务、恢复任务和工作区检查会有更稳定的运行依据。
- 用户确认服务重启后的恢复时，自动导演会记录明确的恢复事件，让任务进度说明这是从中断点继续。
- 规划、拆章、章节执行和修复会更严格地检查已有正文和受保护内容，减少正常流程里静默覆盖用户改动的风险。

同步 beta 修复：AI 主驾的章节自动执行和恢复更稳了。继续任务时会按真实章节结果补跑最早未完成章节，等待审批后继续不会误跳过当前章；服务重启中断的自动导演会进入待恢复提示，由用户确认后再继续，减少后台静默误跑和跳章风险。

- 章节执行区和节奏规划一致时会保留现有数据，但继续时会补跑范围内最早未执行章节。
- 等待审批的章节批次会按普通继续处理，失败后明确允许跳过审校阻断章时才会跳过当前章。
- 服务重启后，仍在排队或运行中的自动导演会标记为待手动恢复，并在任务中心显示恢复入口。
- 用户手动恢复时，系统会直接按当前小说资产判断恢复点，不会再因恢复判断自身递归而失败。
- 质量修复低风险提醒和重规划建议仍会记录通知并继续推进，非 AI 主驾链路保留人工确认边界。
- 开书和接管里的自动推进设置更清楚：默认仍是 AI 自动推进，审批点细项收进高级审批授权，方便区分执行范围和自动确认边界。
- 自动导演失败或取消后，从任务中心按原模型重试也会直接续跑后台任务，减少“已恢复但进度不动”的情况。

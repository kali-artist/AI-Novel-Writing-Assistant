...
AI 主驾的章节自动执行、任务恢复和桌面端跟进入口更稳了。继续任务时会按真实章节结果补跑最早未完成章节，等待审批后继续不会误跳过当前章；服务重启中断或从项目页重新接管的自动导演也会更准确地续跑；任务中心和导演跟进详情会留在客户端内打开，减少人工恢复、跳章、误报缺少拆章和点击入口黑屏的风险。

- 章节执行区和节奏规划一致时会保留现有数据，但继续时会补跑范围内最早未执行章节。
- 等待审批的章节批次会按普通继续处理，失败后明确允许跳过审校阻断章时才会跳过当前章。
- 服务重启后，仍在排队或运行中的自动导演会自动进入恢复续跑。
- 已完成节奏拆章同步的项目重新接管时，会继续使用已有拆章结果进入后续执行。
- 质量修复低风险提醒和重规划建议仍会记录通知并继续推进，非 AI 主驾链路保留人工确认边界。
- 任务中心和导演跟进里的详情入口会在客户端内稳定打开，旧任务详情缺少步骤或里程碑时也会保留可查看状态。

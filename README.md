...
自动导演的起始想法参考更充足：点击“没有想法？”后会生成 5 个临时灵感，方便从更多方向里挑选一个填入输入框。

- 灵感卡片以想法内容为主，不再把方向分类作为显眼标题。
- 5 个灵感会以横向卡片展示，便于快速扫读和选择。
- 生成结果仍然只是临时参考，不会自动保存，也不会自动参与小说生成。
- AI 自动导演进度弹窗会稳定保留流程步骤，不再因为后台轮询刷新后退化成只有全部进展和里程碑历史的视图。
- 正在运行的自动导演任务会优先显示真实推进态；即使历史审批投影还未完全收束，驾驶舱也不会误显示“等待确认”或给出无效确认按钮。
- AI 驾驶舱新增统一展示判断：进度弹窗、小说页提示、任务中心和执行详情会优先使用同一份驾驶舱状态，减少同一任务在不同入口显示不同进度或不同等待状态的情况。
- 自动导演运行中如果仍带有章节标题提醒，提醒不会再把进度弹窗误切成“等待确认”；运行态会继续显示为 AI 接管中。

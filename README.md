...
重大更新：小说基础信息页现在可以直接生成和管理封面主画面，开书后的封面准备不再需要绕到外部工具里单独处理。

- 系统会先根据当前小说信息整理一版封面输入草稿，支持继续用 AI 优化或手动改成最终图片 Prompt。
- 封面图会直接进入当前小说的封面图库；如果本书还没有当前封面，第一张成功图会自动设为主图。
- 任务中心和恢复入口开始识别小说封面任务，恢复时会直接回到这本书的基础信息页。
- OpenAI 图像默认模型切换到 `gpt-image-2`，长时间图片请求的超时处理也更稳。

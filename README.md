...
重大更新：章节执行与自动导演交接开始优先走更轻的审校与更收敛的上下文装配，长链路推进时更省 token，也更不容易因为审核或修复链路过重而中途卡住。

- 仓库当前公开版本已切换为 Apache License 2.0，README、许可证文件和各工作区包信息都已经统一到新的分发口径。
...
- 源码开发的默认安装路径也收顺了。现在普通 `pnpm install` 不会再强制下载 Electron 桌面运行时；只有真正进入桌面端开发时，系统才会按需补拉对应运行时。
- 单章生成、自动执行前 10 章和后续修复链路现在会先走更轻量的审校判断；只有确实发现高风险问题，才会升级到完整审校。连续推进章节时，更不容易被过重的质量检查拖慢，也更适合先把正文稳定跑出来再决定是否深修。
- 章节写作上下文开始按“初稿 / 增补 / 审校 / 修复”阶段区别装配，卷窗口、伏笔账本、最近章节摘要和风格约束不会再在每一轮都一股脑塞满。长篇连续写作时，AI 更容易把注意力放回当前章真正要推进的任务。
- 小说编辑页里的 AI 接管提示改得更贴近真实状态了。像“等待确认”与“需要处理重规划”现在会分开显示；章节待修时也不会再把“继续编辑”错误变成被动卡死，结果区可以直接一键修复，接管完成后也能主动退出当前导演提示。
- 项目设定页把标题辅助入口收回到了标题字段旁的“标题快速选填”；下方折叠区不再重复放一套标题工坊，而是只保留写法确认和本书世界边界整理，开书时的操作更聚焦。

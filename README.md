...
自动导演的节奏 / 拆章、章节执行和驾驶舱状态进一步统一：系统会按真实章节产物判断哪些章节还需要补齐执行资源，取消后的任务和已完成的提醒也会真正退出异常提示。

- 手动细化当前章和自动导演批量细化改用同一套正文执行合同口径，减少“已细化但不能同步到章节执行区”的情况。
- 历史前 10 章状态收口为通用章节批次状态，继续旧任务时会按当前真实产物恢复进度。
- 指定章节范围自动执行完成后，会按本次范围验收正文，不会因为后续章节尚未开写而误报 98% 中断。
- 章节完成后的伏笔账本、读者承诺和角色状态同步会沿用刚收集到的事实，减少 100% 后误报“已中断”的情况。
- 取消自动导演后，小说页主横幅、执行详情和左侧 AI 驾驶舱会按已取消终态收束，不再继续显示“出错需处理”。
- 自动导演完成后，点击“完成并收起”会归档这次完成记录并刷新小说页提醒区。

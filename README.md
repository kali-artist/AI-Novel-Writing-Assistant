...
- 新增“短剧工作台”入口和项目详情工作台，桌面侧栏和移动端创作辅助导航都可以进入；进入后可以通过“来源 → 内容 → 规格”向导创建短剧项目，并在项目内查看素材、策略、分集、台本、质量问题、分镜视频任务和导出入口。项目列表和项目页会显示中文赛道名，不把内部标识展示给用户。待恢复任务不会自动弹窗挡住短剧工作台，需要处理时可从小说列表入口打开。新建时可让 AI 推荐赛道，项目页会按当前进度提示下一步任务，素材页会生成补充问题，质量页会集中汇总待修复台本和可继续质量债；当质量检查发现可修复问题时，会先引导修复该集，再继续分镜和视频任务。分镜视频页会集中展示视频提示词、可选视频通道、任务状态、生成结果入口和失败原因，并可通过通用 HTTP 视频通道接入外部生成服务。

...
重大更新：章节编辑页升级成了更完整的 AI 修正工作台，自动导演和任务恢复也开始直接说清“这次到底会推进哪一卷、哪几章”，让新手更容易看懂当前状态并继续写下去。
- 自动导演、任务中心和小说工作台现在会直接显示当前真实执行范围，而不再一律写成“前 10 章”；接管、恢复和继续执行时，更容易看懂 AI 正在推进的是哪一段章节或哪一卷内容。
- 章节编辑页新增宏观定位、问题卡和推荐任务。AI 会先结合本章在卷内的位置、节奏任务和前后章衔接，告诉你更建议先修哪一块，而不是只给一个空编辑器。
- 正文修改支持自然语言直接告诉 AI 怎么改，并且同时支持片段模式和整章模式。你可以只改当前选区，也可以让 AI 结合整章上下文给出候选版本后再决定是否接受。
- 刚进入章节编辑页时，宏观定位和问题卡区域会先显示明确的 AI 分析加载态，不再像空白或报错。
- 正文区新增段落编号、问题卡定位联动和更稳的选区反馈：可以直接跳到对应段落，高亮后再取消定位，悬浮工具条也会更贴近选区出现，长章精修时更顺手。

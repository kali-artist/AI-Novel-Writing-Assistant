...
模型路由、自动导演、知识库检索和模型厂商接入继续补稳。现在可以按任务配置请求协议和结构化输出格式，也可以给模型供应商设置同模型并发上限和请求间隔；继续节奏拆章或旧自动导演任务时，系统会按真实章节资产确认目标范围内所有章节都已细化，再按单章写作和质量修复依次推进。知识库设置会优先展示新手最需要的 Embedding 服务商、模型和向量库连接，自定义 OpenAI 兼容厂商也可以先拉取模型列表再选择默认模型。
...
- 自动导演前三章连续生成和接管配置继续补稳，章节正文与配套资产保存会更稳。

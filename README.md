...
事实账本（Novel Fact Ledger）：新增 `NovelFactEntry` 表记录已发生不可逆事实，章节接收后自动从义务合同和伏笔指令中提取已完成条目写入，写章时注入 `completedMilestones` 防止 LLM 重复已发生事件，无额外 LLM 调用。

小说生成质量守卫：针对世界设定污染、已完成事件反复重写、场景模式重复和卷节奏失控四类系统性问题，在提示词上下文、共享类型和 Agent 工具三个层面新增守卫机制。

- 世界切片新增防污染约束，禁止将历史时代/异地世界的专有名词带入现代故事章节生成；新增 `rebuild_story_world_slice` 工具修复已污染切片。
- `ChapterWriteContext` 新增已完成里程碑列表（`completedMilestones`），防止 AI 在后续章节反复重追已办好的执照、已签的协议等过程性目标。
- `VolumeWindowContext` 新增卷级关键节点守卫（`keyMilestoneGuards`），防止 LLM 提前写出计划在后续章节才发生的高潮事件。
- `ChapterWriteContext` 新增场景模式黑名单（`recentScenePatterns`），配套写作约束禁止重复使用相同"时间+地点+动作"组合的场景。
- 新增 `audit_chapter_continuity` 诊断工具，对已生成章节正文进行确定性关键词组扫描，检测重复场景模式和开头段落重复。

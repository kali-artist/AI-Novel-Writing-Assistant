...
重大更新：自动导演、卷拆章和任务中心开始围绕“先选方向，再定向修正，再带着可修复提醒继续推进”收拢，开书到返修的中途接力更顺了。
- 自动导演创建页更聚焦：一句灵感就能先补书级 framing，再一起确认关键开书参数、模型和推进方式。
- 方案筛选更可控：会保留历次候选批次，支持只修某一套方向或只重做这套书名，不用每次整批重来。
- 任务中心更会指路：章节标题结构太集中时，会直接给出“打开当前卷拆章”入口；导演任务失败或取消后也能选其他模型继续重试。
- 卷节奏与拆章更稳：节奏板会显式吃进本卷目标章数，拆章时会拦住明显失真的章节跨度，减少一处节奏提示把整卷章数带偏。
- 结构化生成兼容性更好：DashScope / ModelScope Qwen 这类模型在自动导演、卷规划和拆书分析里更少因为 thinking / JSON 边界问题整步中断。

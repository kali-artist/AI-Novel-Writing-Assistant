...
重大更新：自动导演的接管、继续执行和章节批次恢复链路一起补稳了，章节执行本身也收回到更直接的整章写作主链，已有项目更容易从当前真实进度继续往下跑。
- 正文主链更直接：`重写本章` 和普通 `生成本章` 现在都会直接按整章一次性生成，不再先按场景拆开跑，也不再在开写前额外刷新执行合同；如果你现在只想先拿到一章完整正文，等待链路会更短。
- 自动修复止损更明确：单章和批量章节流水线默认只会在初审后自动修一次，再复审一次；如果问题仍然存在，系统会把当前结果留给你继续判断，不会再连续多轮自动修文把时间拖长。
- 批次恢复更贴近真实状态：自动执行章节时，系统会同时看章节当前状态和生成状态来判断哪些章已经处理、哪些章还要继续，恢复旧批次时更不容易误跳还没真正处理完的章节。
- 接管判断更清楚：章节执行区可以直接交给自动导演接管，系统会先判断是恢复当前批次，还是按你现在选的章节范围新开批次，进度文案和继续入口也更贴近真实状态。
- 批次后处理可自己决定：继续自动执行章节时，新增“正文生成后自动审核”和“审核不通过时自动修复”两个开关；想让 AI 只先把正文写出来，还是写完继续审校修复，现在可以按批次单独控制。
- 恢复续跑更不容易跳章：批量章节流水线在重启、恢复或重新挂回时，会按剩余未完成章节继续推进，不再因为前面已完成章节被过滤掉，就把还没写的章节直接跳过去。

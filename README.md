...
项目授权、贡献入口、角色库同步、自动导演开书、模型路由管理和移动端体验继续完善。新手可以更清楚地知道项目使用边界，也能更顺畅地把世界观、角色阵容和模型选择接入长链路创作。

- 自动导演开书弹窗可以直接选择世界观，让导演规划从一开始就遵守对应规则、舞台和边界。
- 模型路由管理支持快速套用到全部任务、异常任务或空白路由，并可一次性保存所有改动。
- DeepSeek V4 Pro 和 DeepSeek Reasoner 会按“思考”开关传递 thinking 配置，结构化输出和关闭思考场景更稳。
- 自动导演进入角色准备时会优先复用可用角色阵容，减少同一本书重复生成角色的情况。

...
角色资源账本、自动导演跟进和可配置部署能力一起进入预发验证。系统会继续帮助你追踪角色手里的关键资源，也会把停住的自动导演任务集中成可处理清单；本地默认保持轻量 SQLite 和本地图片存储，内网部署可以按配置切换 PostgreSQL、企业微信/钉钉通知和 MinIO/S3 图片存储。

- 角色工作台、章节执行、质量修复和卷战略会围绕角色资源账本协同，帮助长篇连载持续追踪道具、线索、身份凭证、底牌和代价。
- 自动导演跟进中心会集中呈现候选确认、重规划、继续自动执行、失败重试和质量修复等待处理任务，并支持直接处理或批量处理。

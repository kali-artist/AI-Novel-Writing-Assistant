...
拆书结果更容易追溯来源，也更适合按自己的学习目标生成：关键结论会显示对应证据摘录，证据可跳回原文章节并高亮摘录上下文；时间线会以带时间提示、阶段和来源片段的节点展示；字段内容过多时会提示按上限保留；创建拆书和单节重跑时可以填写关注重点；拆书页可主动生成或维护深度角色档案，并为角色生成参考图或升格到正式角色库；同一份拆书重复发布到小说知识库时会自动解除旧发布版绑定，减少过时内容干扰后续 RAG 召回。

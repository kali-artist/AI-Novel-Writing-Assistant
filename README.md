...
AI 主驾的章节自动执行更顺了。每章质量未达标时，系统仍会先自动修复一次；如果修复后仍低于阈值，会提醒用户关注结果，但不会把整条流程卡在质量修复检查点。遇到重规划建议时，AI 主驾只记录提醒并继续后续章节，不会自动执行重规划，也不会把流程暂停在重规划检查点。

- 低风险质量提醒会记录通知并继续推进，适合 AI 主驾连续完成章节写作。
- 重规划建议会记录为“重规划提醒已记录”，方便后续回看，但不会误显示成已经自动重规划。
- 企业微信、钉钉和跟进中心会区分自动通过与重规划提醒，减少通知误解。
- 非 AI 主驾的人工审核链路仍保留重规划检查点，方便按阶段确认时先人工处理再继续。

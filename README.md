...
拆书新增预算上限、熔断保护、扩容续跑、发布来源隔离和结构化检索：创建拆书时可以设置本次任务的 token 预算，生成过程中会累计用量；达到上限后任务停止并保留已完成小节。详情页可以调整预算，预算用尽失败后可扩容并续跑未完成小节。拆书发布到知识库时会标记为“拆书发布”，重复发布同一份拆书会追加为同一知识文档的新版本，避免和用户上传文档混合；发布版的题材、卖点、读者、优劣势和章节锚点会进入索引元数据，便于后续创作更精准召回。RAG 召回也会记录后端 trace，为后续召回回放和质量评估准备数据底座。

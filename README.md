...
- 候选生成、候选改写、局部补丁和标题精修改为后台命令执行，界面提交后读取结果展示候选，减少长请求阻塞。
- “确认并继续”和运行策略调整会进入同一条可恢复命令链，后台中断后更容易从统一状态继续。
- 需要 AI 判断的工作区分析和手动编辑影响分析进入后台命令，轻量事实读取仍保持快速返回。
- 章节执行进度改为按章节产物矩阵推导，能区分正文生成、审校、修复、状态提交和可继续范围。
- 单章需要修复时会作为局部可恢复状态展示，不再直接等同于整本失败。
- 任务中心和小说页可以读取更轻量、更可追溯的运行投影，减少固定百分比和旧任务状态带来的进度误判。

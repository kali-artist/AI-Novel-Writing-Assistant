...
- 章节质量评分统一为“重复控制分”：分数越高代表重复越少、质量越好，自动审核和通过判断会按同一口径处理。
- 章节质量闭环开始记录同一失败签名和修复预算，帮助系统识别反复卡住的同一问题，并为局部修复、整章重写、窗口重规划和硬恢复提供依据。
- 写法参考会先净化成可迁移写法指导，源作品角色名、地名和专有称谓不会直接进入正文上下文；如果审校发现这类泄露，会自动整章重写。
- LLM 调用路由开始按写作、轻审、严格审校、修复、重规划和状态解析分层记录；没有单独配置更强模型时，会标记为降级执行，方便排查成本和质量问题。
...
- 整本进度优先按“可继续章节 / 总章节”计算，任务面板会拆出规划、章节、质量和当前动作进度，减少单个后台任务进度误导。
- 章节队列会按最新质量闭环派生下一步，复检可继续的章节不会继续残留“一键修复”。
...
- 全书自动推进恢复后会继续保持自动导演策略。只有普通状态提案、没有审校硬问题时，系统会继续章节写作；真正需要审核的风险仍会阻断，且不会提前留下“生成中”的章节状态。

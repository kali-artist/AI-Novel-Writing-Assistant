...
模型路由、自动导演、知识库检索、桌面升级和小说列表继续补稳。可以按任务配置请求协议和结构化输出格式，也可以给模型供应商设置同模型并发上限和请求间隔；继续节奏拆章或旧自动导演任务时，系统会按真实章节资产确认目标范围内所有章节都已细化，再按单章写作和质量修复依次推进。首页、小说列表和卷规划版本加载更轻快，小说列表也可以直接进入章节预览。

- 模型路由支持选择自动、OpenAI 兼容或 Anthropic 协议，也支持指定结构化输出格式。
- 选择 Anthropic 协议时，结构化格式固定为提示词 JSON，页面和后端都会阻止不适用的原生 JSON 组合。
- 连通性检测会同时验证协议和结构化格式，并把有效组合写回模型路由偏好。
- 服务端会在发送模型请求前拦截空消息、空批次和空白内容，排查失败原因更直接。
- Anthropic 原生协议已接入普通调用、结构化调用和 JSON 修复调用。
- 自动导演进入待确认、异常、恢复或完成等状态时，会重新按配置发送企业微信和钉钉跟进通知。
- 自动导演从节奏 / 拆章阶段接管时，会先重置章节执行、质量修复和旧待确认检查点，再重新补齐目标范围内的节奏规划与章节细化。
- 自动导演会在目标范围内全部章节细化完成后再进入章节执行，并按单章完成写作与质量修复后继续下一章。
- 旧任务留下的 `front10_ready` 或自动执行状态不会再绕过真实章节检查；范围内只细化了部分章节时，会回到节奏 / 拆章补齐细化。
- 全书自动导演会校验全书章节细化完成度，并清理执行区里和最新节奏拆章不一致的旧正文、旧完成状态和多余章节。
- 模型供应商可设置同模型并发上限和请求间隔，`0` 表示不限制，便于为高失败率渠道做配置化限速。
- 分卷、章节细化和故事规划写入会使用更充足的事务时间，并补齐 PostgreSQL 与 SQLite 两套数据库的必要字段和索引。
- 知识库检索设置优先展示 Embedding 服务商、Embedding 模型和向量库连接，集合命名、切块、召回、超时和任务参数收进高级配置。
- Embedding 服务商可跟随已配置的内置或自定义模型厂商加载，向量库 URL 支持 Qdrant Cloud、自托管 Qdrant 和本机地址。
- 知识库运行状态支持清理已结束索引任务，排队中和执行中的任务会保留，避免误删正在运行的任务。
- 自定义 OpenAI 兼容模型厂商可以先拉取模型列表再选择默认模型，本地或免密服务配置更顺。
- 桌面端升级后会继续使用本地数据目录里的小说数据库，升级启动不会因为本地数据库地址缺失而停在启动页。
- 首页优先读取最近项目，小说列表不再一次带出完整大纲、结构化大纲、书籍圣经和书级契约，进入页面时等待更少。
- 卷规划历史版本会先显示摘要，选中某个版本时再读取完整内容，历史版本较多时也更稳。
- 小说列表新增预览入口，可以按章节阅读已生成正文、查看总字数和章节状态。
- 自动导演前三章连续生成和接管配置继续补稳，Windows 桌面版打包运行时也会使用更明确的 SQLite 配置。

...
整合 0.3 系列累积的 69 个改动一次性发布。重点：

- 自动导演全自动成书每批章节后自动确认 pending 角色，角色一致性漂移修复；LLM 限速器内存泄漏修复。
- 章节生成上下文按本章参与者精准筛选角色资源账本，高风险已入账与待确认提案分离。
- 自动导演新增四种运行模式说明（先准备到可开写 / 全书自动成书 / 按范围执行 / 正文后去 AI 检测与修正）及中断条件。
- 拆书新增角色深度档案、形象演变、章节证据回溯、双栏阅读、范围定向分析、token 预算守卫与稿件诊断。
- RAG 流式并行索引、并发可调、retrieval trace、拆书 facets 入索引、chunk hash 去重。
- 漫画工作台新增场景一致性、角色视觉资产、视觉锚点和图像生成确认；短剧改编生产管线 v3。
- GitHub Pages 公开介绍站上线（端口 4173），新增 33 篇用户向文档与生产链深度文档，文档站支持搜索、TOC、面包屑、GFM 表格、提示块和上下文翻页。
- 公开介绍站和文档把所有内部 `snake_case` 阶段 key 替换为中文名，开发者别名对照表保留在自动导演阶段全景文末。
...
- 自动导演新增「运行模式」完整说明：先准备到可开写、全书自动成书、按范围执行、正文后去 AI 检测与修正四种模式的差异、适用场景和切换方式；全书自动驾驶的中断条件首次写入文档。
- 第一本小说实操路径新增「第一步半：选择运行模式」分流，第一本书统一推荐「先准备到可开写」。
- 公开文档里全部内部 `snake_case` 阶段 key（如 `candidate_seed_alignment`、`volume_skeleton` 等）替换为中文名「灵感对齐」、「卷骨架」等；开发者可在自动导演阶段全景文末的「技术别名对照表」查到原 key。
- 开发计划重写为"已完成（近 3 月）/ 进行中 / 下一步关注"三段，列出真实在做的能力（漫画改编、短剧改编、章节编辑器 v2、角色资源账本、自动导演执行面隔离等）。
- 常见问题补充四条高频问答：运行模式怎么选、全书自动驾驶中途停下怎么办、介绍站端口是否会和主程序冲突、桌面版数据存在哪。
- 使用方法页重写为按身份导航的索引（第一次跑通 / 想理解机制 / 任务卡住 / 想做衍生）。
- 项目介绍补充"衍生工坊"层（漫画 / 短剧工作台），并明确"主链跑通前不建议打开"。
- 修复 GFM 表格、删除线、任务列表不渲染的问题，实操路径和深度文档的阶段对照表恢复正常显示。
- 修复模块文档（题材基底库、推进模式库、写法引擎等）截图不显示的问题，新增图片不再需要手动登记白名单。
- 修复文档右侧"本文目录"点击后跳回首页、不滚动到对应段落的问题。
- 公开介绍站顶部品牌图标和浏览器 favicon 改为项目实际应用图标。

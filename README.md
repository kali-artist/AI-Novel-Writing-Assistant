...
- 新增“短剧工作台”入口和项目详情工作台，桌面侧栏和移动端创作辅助导航都可以进入；进入后可以通过“来源 → 内容 → 规格”向导创建短剧项目，并在项目内查看素材、策略、分集、台本、质量问题、分镜视频提示词和导出入口。新建时可让 AI 推荐赛道，项目页会按当前进度提示下一步任务，素材页会生成补充问题，质量页会集中汇总待修复台本和可继续质量债。

...
- 章节规划会更稳地识别模型返回的目标字段。即使模型把目标写成 `goal`、`chapterGoal` 或中文“章节目标”，也能继续整理成可执行规划，减少正文生成前被字段命名漂移卡住。
- 如果模型服务或中转站返回的是 HTML 错误页，系统现在会更准确地提示这是传输或服务异常，不再把这类问题误判成普通 JSON 格式错误。

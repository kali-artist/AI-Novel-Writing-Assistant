...
角色资源账本、图片生成和自动导演提醒都更稳了：写章时会区分高风险已入账资源和真正待确认变更，只带入本章相关资源；漫画和短剧工作台的图片生成增加确认与参考素材链路；自动导演暂停等待处理时，可以通过浏览器桌面通知提醒你回到跟进中心。

- 角色资源账本会分别提示“高风险已入账资源”和“待确认资源变更”，避免把谨慎参考误当成用户待确认事实。
- 资源变更确认会统一提交到账本、快照和事件历史，归属冲突、状态复用冲突和可见性回退会进入待确认。
- 章节写作上下文会按本章参与角色和资源使用窗口裁剪，减少无关角色物品挤占 prompt。
- 漫画和短剧图片生成接入确认式流程，生成前能更清楚地处理参考图、提示词和目标产物。
- 漫画角色、场景、资产和分格图片的参考素材链路更清晰，方便检查当前图用了哪些参考。
- 系统设置新增“自动导演暂停提醒”。授权浏览器通知后，自动导演等待确认、需要恢复或被校验拦住时会发送桌面通知。

...
彻底修复了正文即兴写出的硬设定（交易性质、金额、票号、数量等）无法跨章保持一致的问题；同时短剧创作工作台继续补齐新手引导，可以从小说、原创灵感或导入文本进入短剧策略、分集、台本、质量检查、分镜和视频提示词流程。
...
- 新增“短剧工作台”入口和项目详情工作台，桌面侧栏和移动端创作辅助导航都可以进入；进入后可以通过“来源 → 内容 → 规格”向导创建短剧项目，并在项目内查看素材、策略、分集、台本、质量问题、短剧角色资产、分镜视频任务和导出入口。角色页围绕出镜功能、观众识别、固定造型、表演声音、台词规则和冲突关系组织，不再按小说人设表呈现。项目列表和项目页会显示中文赛道名，不把内部标识展示给用户。待恢复任务不会自动弹窗挡住短剧工作台，需要处理时可从小说列表入口打开。新建时可让 AI 推荐赛道，项目页会按当前进度提示下一步任务，素材页会生成补充问题，质量页会集中汇总待修复台本和可继续质量债；当质量检查发现可修复问题时，会先引导修复该集，再继续分镜和视频任务。分镜视频页会集中展示视频提示词、可选视频通道、任务状态、生成结果入口和失败原因，并可通过通用 HTTP 视频通道接入外部生成服务。

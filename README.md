...
重大更新：自动导演恢复链路、章节标题生成和错误提示一起补稳了。任务中断后更容易从真实进度继续，长卷拆章也开始按卷节奏逐段生成，不再把整卷几十章一次压给模型。
- 自动导演和任务恢复更顺：任务中心、首页和小说工作台会更明确地区分当前是在恢复、可继续接管，还是需要重新处理；页面刷新、切页或服务重启后，也更不容易把你带回错误入口。
- 整本推进更贴近当前真实内容：章节执行、状态沉淀、后台恢复和重规划现在会更多参考已经写出的章节事实与最新阶段结果，减少前后状态打架、看起来能继续却接不到正确步骤的情况。
- `节奏 / 拆章` 现在按当前卷的节奏段逐段生成章节标题与摘要；章节多的时候，后半段标题失真、摘要发虚或整卷一起失准的问题会更少。
- 当前卷章节列表会显式绑定到各个节奏段，并支持只重生单个节奏段；如果只是某一段标题不满意，不必整卷重跑，局部重写就可以。
- 节奏工作区会直接告诉你每个节奏段当前是 `待生成 / 生成中 / 已生成 / 需重试`，更适合边看边确认当前卷到底还缺哪一段。
- 右上角错误提示改成手动关闭了。出错信息不会自己消失，排查失败原因或模型返回异常时能一直留在屏幕上。

...
重大更新：`节奏 / 拆章` 和自动导演的恢复点收细到了“最近已完成边界”，中途失败后不再默认从 0 整段重跑，继续生成时更接近你刚才停下的位置。
- 自动导演在节奏板、章节列表和章节细化阶段，现在会按真实已完成进度恢复：已经完成的卷、节奏段和章节细化项会直接跳过，只继续未完成的那一部分。
- `节奏 / 拆章` 里的整卷拆章现在支持从第一个未完成节奏段继续，不会因为中途失败、换模型重试或服务重启，就把前面已经拆好的节奏段整卷清空重来。
- 批量章节细化重试也改成只补当前缺失的 `章节目标 / 执行边界 / 任务单`，已经补齐的部分会直接沿用，减少重复等待和重复改写。
- 当节奏板、拆章或整卷生成在中途报错时，页面会先检查后端最近自动保存的进度；只有确认后端快照已经前进，才会回填到当前工作区，避免把你本地尚未落盘的草稿误覆盖。

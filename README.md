...
角色资源账本、自动导演下一版和可配置部署能力一起进入预发验证。系统会继续帮助你追踪角色手里的关键资源，也会把停住、异常、需校验或正在自动推进的导演任务集中成更清楚的处理清单；本地默认保持轻量 SQLite 和本地图片存储，内网部署可以按配置切换 PostgreSQL、企业微信/钉钉通知和 MinIO/S3 图片存储。

- 角色工作台、章节执行、质量修复和卷战略会围绕角色资源账本协同，帮助长篇连载持续追踪道具、线索、身份凭证、底牌和代价。
- 自动导演跟进中心会按待处理、自动推进、异常、已替代和需校验分区呈现任务，并支持低风险批量继续、异常批量重试和只读重新校验。
- 需校验任务支持一键安全修复；系统只处理低风险状态对账、检查点、进度、恢复目标、替代原因和审计记录，遇到清正文、重写、重规划、候选确认或模型切换会要求人工处理。
- AI 推进自动通过审批点时，会在自动推进分区留下最近通过记录，并通过企业微信/钉钉发送“AI 已自动通过并继续推进”的审计通知。
- 开书自动执行可以选择全书、前 N 章或前 1 卷；书本页接管也能按全书、章节范围或卷范围限制开始节点，范围执行更贴近真实目标。
- 书本页继续已有进度会保留已有资产，并把同一目标范围内被接管的旧任务标明为“由本任务替代”，减少误把正常替代看成异常取消的情况。
- 自动导演重新生成前会先创建重写前备份；如果备份失败，本次重写会停止，不会继续清除内容，任务详情和版本历史可用于找回重写前内容。
- 章节批次和质量修复会按结构化风险分级决定下一步：低风险且已授权自动推进时继续章节执行，重规划或大范围返工会停下来等待人工确认。
- 系统设置新增自动推进偏好；选择 AI 推进时会先带入默认允许自动通过的审批点，开书和接管时还可以按本书单独调整。
- 自动导演长链路增加跨进程高内存保护、范围校验和假运行自愈；同一本书同一范围的自动导演或卷规划生成会被拦截，直连卷生成也会减少大响应体传输。
- 企业微信和钉钉通知可按配置接入自动导演跟进，让需要人工确认或继续执行的任务更容易被及时看到。
- 数据库默认继续使用 SQLite；配置 PostgreSQL 后，角色资源账本、自动导演跟进日志和相关数据结构会保持同等支持。
- 图片资产默认继续保存在本地文件中；只有显式选择 `s3` 或 `minio` 时才启用 MinIO/S3 存储。

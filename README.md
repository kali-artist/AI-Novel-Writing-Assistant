...
重大更新：Windows 桌面版已经推进到可分发的 Beta 阶段。现在可以产出 `Setup.exe` 和 `portable` 两种产物，安装后不需要再自己准备 Node、pnpm 或 Prisma 环境；桌面端也补上了中文启动壳、日志兜底、旧本地数据库手动迁移入口和 GitHub Releases Beta 更新链，整体更接近“安装即可直接进入创作”的使用方式。
- 桌面端启动不再先给长时间白屏，安装器也支持手动选择安装目录；应用图标、启动界面、快捷方式和安装器资源已经统一，不再残留 Electron 默认图标。
- 桌面版和原本的 Web 本地数据会默认分开存放，但系统设置里已经提供旧 `dev.db` 的手动导入入口，迁移时会先备份当前桌面数据库，再接管旧数据并自动重启。
- 缺少模型配置时，桌面端会明确引导回现有设置页完成模型与密钥配置；同时支持图片生成的厂商现在可以直接在设置里补齐图片模型、API 地址和密钥，不必再单独修改 `.env`。
- 安装版自动更新链已接入 GitHub Releases Beta，知识库旧 `.env` 配置也开始兼容当前设置体系；知识库检索设置补成中文后，新手排查 Embedding / 检索参数会更容易。

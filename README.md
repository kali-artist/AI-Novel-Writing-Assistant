...
项目授权口径已调整为 AGPLv3 + 商业授权说明；角色库和小说内角色也开始分清“可复用资产”和“本书剧情实例”。一本小说里的当前状态、关系进度和章节结果不会自动污染角色库，也不会自动影响其他正在使用同一角色的小说。

- 项目默认采用 GNU AGPLv3 授权，继续保留可学习、可修改、可按开源协议合规使用的路径。
- 服务型商用口径已明确：将本项目或修改版本作为后端，以 SaaS、托管或其他形式向第三方提供服务，需要先取得作者商业授权。
- 小说角色可以保存为角色库角色，系统会先拆分可复用设定、本书专属剧情状态和需要确认的高风险内容。
- 角色库角色可以引入到当前小说，并保留来源版本和引用关系，方便后续查看是否要应用角色库更新。
- 角色库更新只会生成可选同步建议，不会自动改写任何小说；用户可以选择应用、忽略或让当前小说保持独立。
- 角色同步会用 AI 结构化判断基础身份、稳定人格、本书适配、运行状态和可沉淀成长，减少新手手动判断边界的负担。
- 角色系统升级路线进一步明确，后续会围绕角色叙事岗位、关系张力和章节角色上下文包推进，让角色持续进入章节写作、审稿和修复。
...
提交 Pull Request 即表示你确认自己有权提交该内容，并同意该贡献可随项目按 AGPL-3.0-only 默认开源协议分发，同时可纳入项目维护者另行提供的商业授权；如果包含第三方代码、素材或其他受许可证约束的内容，请在 PR 中明确说明来源和许可证。详见 [CONTRIBUTING.md](./CONTRIBUTING.md)。
...
贡献说明：新贡献默认按 AGPL-3.0-only 提交，并可纳入项目维护者另行提供的商业授权，详见 [CONTRIBUTING.md](./CONTRIBUTING.md)。

...
重大更新：自动导演继续、恢复和已有项目接管已进入更稳定的后台执行模式，长任务推进时页面仍可查看任务中心、项目导航和当前状态。

自动导演恢复和范围执行更稳了。已有小说接管后，退出再进入导演模式会按真实任务阶段继续展示；服务重启后的待恢复任务会快速进入后台恢复，不再长时间卡住页面或弹窗。

- 点击继续自动导演后，页面会快速收到已提交结果，后续由后台继续推进，减少整页接口挂起和浏览器请求堆积。
- 自动导演运行中的状态展示改为轻量进度读取，运行时不会持续强拉完整卷工作区，刷新和切换页面更顺畅。
- 等待确认的步骤会明确按用户确认继续，只放行当前停留点；后续新的高风险操作仍会停下来提示确认。
- 章节标题修复会进入后台命令队列，提交后快速确认已接收，不再直接阻塞页面或立刻强刷完整卷工作区。
- 自动导演后台执行失败时会同步收束运行中的步骤，并把任务显示为等待手动恢复，减少失败后仍像排队或运行中的错位。
- 自动导演继续旧任务时会更稳地保存产物依赖，减少从章节细化、拆章或质量修复处恢复时因依赖记录顺序导致的失败。
- 继续前 10 章自动执行时，系统会按用户选择进入章节写作；已具备执行资源的旧任务不会误回到节奏 / 拆章重新推进，也不会在章节执行入口再次停回等待确认；资源不完整时仍会先补齐规划。
- 已有小说接管会先进入后台分析和接管任务，不再把接管过程变成前台长等待。
- 服务重启或后台执行中断后，任务会优先显示可解释的待恢复状态，减少“看起来运行中但实际已暂停”的错位。
- 待恢复任务点击恢复后会先从弹窗中移除，并继续在后台推进；任务状态和小说列表会异步刷新。
- 小说工作台左侧流程状态会优先跟随自动导演当前阶段，不会因为已有旧卷战略资产就把后续步骤提前显示为已完成。
- Artifact Ledger 恢复会复用已有产物和依赖记录，减少重复恢复、并发恢复或同一依赖重复入账导致的冲突。
- 规划恢复链会按真实缺口继续，已有卷规划时也不会跳过缺失的故事宏观规划、书级约定或角色资产。
- 自动导演继续、恢复和章节流水线会复用已有运行记录与产物依赖，减少重复点击或服务重启后的重复写入。
- 自动导演运行时账本会按步骤实际产物记录来源，章节写作、审校、状态同步和伏笔 / 角色同步不会互相污染产物归属。
- 自动导演恢复样本可以通过只读检查快速归类，方便验证接管、待恢复、章节批次和改文影响回归；已有正文缺少章节正文账本基线时也会直接提示。
- 章节批次恢复会以真实正文是否存在作为完成依据，避免把已标记修复但正文为空的章节误判为完成。
- 自动导演写入合同进一步统一，候选、确认、接管、规划、拆章、章节写作、审校、修复和状态提交会走同一套步骤合同与策略判断。
- 故事宏观规划和书级创作约定拆成独立恢复节点，缺哪一步就从哪一步继续。
- 自动导演下一轮最高优先级已收敛为 13 项，优先推进真实数据恢复、产物账本、策略门禁、质量闭环和状态驱动重规划。
- 章节标题生成与修复会主动降低同卷标题重复风险，让拆章结果更适合直接进入正文执行。
- 确认方案、已有项目接管、结构化拆章、章节执行和恢复链路继续收进统一运行时，为后续 P0 正常流程开发打下更稳定基础。

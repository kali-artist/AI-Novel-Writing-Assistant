...
拆书结果更容易追溯来源，也更适合按自己的学习目标生成：关键结论会显示对应证据摘录，证据可跳回原文章节并高亮摘录上下文；时间线会以带时间提示、阶段和来源片段的节点展示；字段内容过多时会提示按上限保留；创建拆书和单节重跑时可以填写关注重点；拆书页可主动生成或维护深度角色档案，并为角色生成参考图或升格到正式角色库；也可以把自己的小说正文导出为知识文档后创建诊断拆书；同一份拆书重复发布到小说知识库时会自动解除旧发布版绑定，减少过时内容干扰后续 RAG 召回。

- 拆书小节的证据会尽量绑定到具体关键结论字段，字段旁会显示来源提示，方便判断结论依据。
- 源文档版本会按章节缓存；证据匹配到原文摘录时，可以在证据面板查看对应章节上下文并高亮摘录。
- 故事时间线会用结构化节点保存关键事件、时间提示、阶段标签和来源片段，历史字符串时间线也能继续显示。
- 时间线关键结论会按阶段聚合展示，发布和续写参考里的时间线维度也会使用中文字段名。
- 结构化结论数组超过上限时，会提示对应字段，帮助判断哪些结论经过截断展示。
- 全量拆书会先生成总览，再让后续小节参考总览定位，减少剧情、人物、世界观等分析互相打架。
- 创建拆书时可以填写“本次拆书重点”，每个小节也可以填写“本节特别关注”，让分析更贴近当前想学习或复用的问题。
- 单节重新生成剧情、人物等小节时，会继续参考已生成的总览定位，减少局部重跑后口径漂移。
- 拆书页新增“角色档案”区域，可主动生成深度角色档案，也可手动添加、编辑或删除角色；角色弧线和代表性场景会和人物基础信息一起展示。
- 深度角色档案需要主动触发，不随全量拆书自动生成，避免默认拆书成本突然增加。
- 深度角色档案可以生成角色参考图、设置主图和删除图片；升格到正式角色库时可默认携带主图，后续作为独立角色素材继续创作。
- 拆书页新增“诊断稿子”入口，可选择自己的小说，导出当前章节正文并创建诊断拆书，用同一套分析框架检查节奏、人物、主题和伏笔问题。
- 历史拆书证据没有字段绑定时仍会正常展示，不需要重新生成。
- 同一拆书反复发布到同一本小说时，只保留该拆书最新发布版的知识库绑定；旧发布文档本体不会被删除。

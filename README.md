...
自动导演重构进入 MVP 落地阶段。新建、候选确认、已有项目接管和手动继续开始沉淀到同一份运行状态里，系统会先分析当前小说资产，再按统一策略决定下一步怎么推进；后续创作中枢也可以通过同一组入口读取状态、切换策略和继续任务。
...
- 自动导演新增运行时状态、策略切换和继续入口，方便后续接入创作中枢。
- MVP 文档补充了已落地内容与下一轮章节执行、角色治理、世界观和知识库主链化的边界。

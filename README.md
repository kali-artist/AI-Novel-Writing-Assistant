...
自动导演的后台执行、任务投影和继续接管链路进一步收口；小说页与任务抽屉现在能看到更一致的运行状态，暂停或中断后也更容易接上已有进度。

- 候选生成、方向细化、书名优化、接管继续、工作区分析和手动编辑影响分析进一步统一到后台命令链，减少不同入口各自维护执行状态带来的错位。
- 小说页和任务抽屉开始读取更完整的导演运行投影，能更直观看到当前动作、后台资源状态、章节执行进度和需要处理的风险。
- 章节执行进度改为按实际章节产物推导，能区分合同准备、正文生成、审校、修复、状态提交和可继续范围。
- 自动导演继续执行时会优先结合当前工作区内容与已保存运行上下文恢复，减少恢复错位和重复推进。
- 书级规划、角色准备和分卷策略开始优先按真实产物判断是否已完成，用户手动调整后，系统更容易按当前内容决定是否跳过或回补步骤。

...
拆书从创建到阅读都更适合按目标推进：新建拆书可以选择全文、章节范围或按字数换算出的章节范围；结果页任务操作固定在顶部工具栏，小节 Tab 更早进入首屏；宽屏可打开双栏对照，一边看原文章节，一边核对拆书结论和证据。知识库上传也改为弹窗入口，文档列表更聚焦。

- 新建拆书支持选择原文范围，可以分析全文、指定章节起止范围，或输入 `5k`、`5万` 这类字数范围并由系统换算为章节边界。
- 顶部 sticky 工具栏承载复制、重新生成、发布、任务中心、导出、生成写法、归档和双栏入口，滚动阅读时不用回到页面顶部找操作。
- 分析概要、发布到知识库和运行元信息默认折叠，首屏优先露出小节 Tab 和关键结论。
- 证据按小节内嵌为字段 chip，优先显示绑定的结构化字段和数组下标；带章节定位的证据会继续展示原文高亮片段。
- 宽屏双栏模式会在左侧显示原文章节，章节目录和正文并排阅读；点击右侧证据可跳到对应章节并高亮摘录，左侧章节变化后右侧相关字段会标出“本章”。
- 知识库文档上传改为弹窗入口，上传成功后回到文档列表查看索引状态。

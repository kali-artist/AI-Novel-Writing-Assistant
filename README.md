...
- 视频提示词、首帧图和角色设计稿重生成会保留历史版本；工作台会优先使用当前版本创建视频任务和导出剪辑草稿，也能打开旧版图片回看构图与造型变化。
- 短剧策略会生成可执行的付费卡点计划，分集大纲和质量检查会按首付费集、强度曲线和卡点阈值推进。
- 质量页可以批量做平台合规预检，提前发现常见短剧平台驳回风险；阻断问题会进入修复队列，提醒项不阻断继续生产。

...
- 自动导演 LLM 运行优化收口：约 82%。
- 自动导演恢复 / 继续主链：约 95%。
- `full_book_autopilot` 从一句话想法到连续章节生成主链：约 91%。
- 面向新手的 P0 整本成书目标：约 74%。
...
- 继续把全书级质量预算从 circuit breaker 计数扩展为 `issueSignature + blockingLedgerKeys + affectedChapterWindow` 的更细账本，让跨章节重复问题的归并更准确。

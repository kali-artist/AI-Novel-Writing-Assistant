...
- 全书质量债继续策略：`full_book_autopilot` 遇到重复重规划或自动修复耗尽时，不再直接停给新手用户，而是把当前章登记为质量待回收并继续推进后续章节；相关事件会进入运行时间线和任务种子，避免恢复后重复消耗同一轮 LLM。
- 循环计数状态收口：自动执行状态会保留 circuit breaker 计数、质量债章节和质量债摘要，避免继续/恢复后把同类问题重新当成第一次失败处理。
- 全书级质量预算账本细化：自动执行状态已保留 `qualityLoopLedger`，同一问题会按 `issueSignature + blockingLedgerKeys + affectedChapterWindow` 归并，记录局部修复、整章重写、窗口重规划和暂存继续次数；Worker 恢复后会沿用这份账本，不把同一轮问题重新当作首次失败。
- 半自动确认边界修正：低风险质量提醒在手动/半自动模式下会走显式授权或保留确认，不再绕过用户选择；全书自动成书模式仍按“普通问题自动处理”推进。
- 状态提案 AI 自动解析闭环：新增 `director.state_proposal_resolution` PromptAsset，`full_book_autopilot` 遇到普通信息披露、关系变化和角色资源提案阻断时，会先由 AI 解析为自动应用、暂存归档、窗口重规划或人工恢复；可信普通提案会自动写回状态，冲突提案会进入重规划，低置信度和高风险覆盖仍停到硬恢复。
- 质量债可解释展示：任务面板会显示“质量待回收”的章节范围和后续处理方式，用户能看懂系统是在先推进整本书，而不是无声跳过问题。
- 单章质量预算展示：任务面板、自动导演进度卡和 AI 工具状态会读取同一份 `qualityBudgetSummary`，展示当前章节的局部修复、整章重写、窗口重规划用量，以及同类问题下一步会继续修复、重写、重规划还是进入质量待回收。
- 章节下一步动作收口：章节质量闭环判定可继续时，章节卡片会优先显示“继续下一章”或“写本章”，避免质量可继续的章节仍露出“一键修复”。
...
- 自动导演恢复 / 继续主链：约 97%。
- `full_book_autopilot` 从一句话想法到连续章节生成主链：约 95%。
- 面向新手的 P0 整本成书目标：约 78%。
...
- 用真实长链路样本检查质量待回收队列的最终回收效果，确认“先继续整本、后集中修复”的策略不会让后续章节继承不可接受的状态偏差。

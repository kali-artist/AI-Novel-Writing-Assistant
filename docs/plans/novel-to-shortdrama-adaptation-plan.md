...
截至 2026-06-09，本计划已推进到后端 Phase 6 纵向骨架：短剧项目、三类内容源、防腐层、策略、分集、台本、质量闸、修复、导出、角色库、分镜、视频提示词和 mock VideoProvider 已具备 API 与类型验证。完整前端工作台和真实视频 provider 仍未完成。
...
| 前端工作台 | API 基础已接 | `client/src/api/drama.ts` 和 query keys 已补齐；完整 `/drama` UI 尚未实现。 |
| 分镜/视频/剧本保真 | 分镜与视频提示词已到后端骨架 | 已新增分镜、镜头、视频提示词和 mock VideoProvider。剧本保真层仍未实现。 |

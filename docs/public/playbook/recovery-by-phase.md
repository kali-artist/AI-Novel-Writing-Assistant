...
- `candidate_selection_required`：等待选方向。
- `character_setup_required`：等待确认角色。
- `volume_strategy_ready`：等待确认卷规划。
- `chapter_batch_ready`：等待进入章节执行。
- `replan_required`：等待处理质量修复或重规划。
...
| 创建小说 | 确认后项目没出现 | 任务中心 | 重试 `confirm_candidate` | 只有重复数据冲突且无法修复时人工处理。 |
...
| 节奏板 | 节奏节点覆盖不足 | 任务中心、卷规划 | 重生成 `beat_sheet` | 节奏目标和卷战略冲突时回卷战略。 |
| 章节清单 | 章节数不对、标题错乱 | 章节列表 | 重生成 `chapter_list` | 节奏板本身错时先回节奏板。 |
| 章节同步 | 章节任务没写入章节 | 任务中心 | 重试 `chapter_sync` 或继续结构化拆章 | 不常需要重规划。 |
| 章节细化 | 某些章节没有任务单 | 任务中心、导演跟进 | 从最近进度继续 `chapter_detail_bundle` | 如果章节清单错误，回章节清单。 |
| 章节执行 | 正文生成失败或空 | 任务中心、章节页 | 重试、换模型、检查上下文 | 章节任务与大纲冲突时重规划局部。 |
| 审核修复 | 修复失败或质量债务 | 章节页、导演跟进 | 轻修复、记录债务、继续 | 明确 `replan_required` 且影响后续时。 |
...
全书自动执行和 `continue` / `resume_from_checkpoint` 命令有自动恢复次数；超过次数后会转为手动恢复。

...
### 2026-06-25（拆书结果页首屏精简）

拆书结果页更适合持续阅读和核对：任务操作固定在顶部工具栏，分析信息、发布和运行元信息默认折叠；小节 Tab 更早进入首屏，证据从独立长面板收进对应小节内，用户可以直接从关键结论旁查看字段绑定证据和原文摘录。

- 拆书详情页顶部新增跟随滚动的工具栏，复制、重新生成、任务中心、导出、生成写法和归档入口不再占据大块信息卡。

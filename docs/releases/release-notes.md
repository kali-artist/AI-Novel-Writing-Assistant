...
重大更新：小说基础信息页现在可以直接生成和管理封面主画面，开书后的封面准备不再需要绕到角色图或外部工具里单独处理。

- 在小说编辑页的基础信息区，可以先根据当前书名、简介、卖点、目标读者、世界氛围和推进模式自动整理一版封面输入草稿，再继续用 AI 优化或手动改成最终图片 Prompt。
- 封面生成会默认按竖版主画面思路出图，生成结果直接进入当前小说的封面图库；如果本书还没有当前封面，第一张成功图会自动设为主图。
- 已有封面后，可以在同一本书里查看候选图、切换当前封面和删除旧图；删除当前主封面时，系统会自动补一张新的主图，避免封面状态丢失。
- 任务中心和恢复入口开始识别“小说封面”任务，恢复时会直接回到这本书的基础信息页，而不是落到无关的图片或角色入口。
- OpenAI 图像默认模型切换到 `gpt-image-2`，图像请求体也补齐了当前兼容接口常见字段和更稳的超时处理，减少长时间出图时无提示失败的情况。

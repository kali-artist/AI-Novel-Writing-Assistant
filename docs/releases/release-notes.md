...
- 章节局部修复更稳。局部补丁会在精确匹配失败后尝试确定性的空白等价匹配；无法唯一命中、多处命中或替换无效果时不会强行改文，而是记录待修复问题。
- 章节修复失败不再轻易卡死整本书。局部补丁失败但正文仍可保留时，系统会记录待修复状态并继续推进后续章节；正文为空、保存失败、核心结构缺失等不可恢复问题仍会暂停让用户处理。

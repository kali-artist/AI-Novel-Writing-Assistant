...
### 2026-06-24（拆书证据溯源、时间线结构化与重复发布清理）

拆书结果更容易追溯来源，也更容易用于续写参考：新生成的拆书会把证据绑定到对应的结构化结论字段，查看关键结论时可以直接看到支撑摘录；时间线会以结构化节点保存时间提示、阶段和来源片段；结构化结论字段内容过多时会提示哪些字段按上限保留；同一份拆书重复发布到小说知识库时，会自动解除旧发布版绑定，减少后续 RAG 召回过时拆书内容的概率。

...
### 2026-06-24（拆书证据溯源、截断提示与重复发布清理）

拆书结果更容易追溯来源，也更容易判断信息是否完整：新生成的拆书会把证据绑定到对应的结构化结论字段，查看关键结论时可以直接看到支撑摘录；结构化结论字段内容过多时会提示哪些字段只保留了前 12 项；同一份拆书重复发布到小说知识库时，会自动解除旧发布版绑定，减少后续 RAG 召回过时拆书内容的概率。

- 拆书小节的证据会尽量绑定到具体关键结论字段，字段旁会显示来源提示，方便用户判断这条结论来自哪段原文摘录。
- 拆书结构化结论数组超过 12 项时，会在关键结论区提示对应字段，帮助用户知道哪些内容经过截断展示。
- 新生成拆书时，模型会收到数组上限说明，优先按重要度排序保留更值得复用的结论。

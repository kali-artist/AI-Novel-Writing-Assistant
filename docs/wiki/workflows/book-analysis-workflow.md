...
- 服务端拆书生成的外部稳定入口是 `bookAnalysis.generation.ts`。该文件应保持 orchestration facade 职责，把生命周期、输入范围、总览上下文、章节缓存和小节优化预览分别委托给 `services/bookAnalysis/generation/` 下的责任模块；新增生成能力时优先扩展对应责任模块，避免把 Prompt 调用、缓存、状态流转和 HTTP 可见副作用重新堆回 facade。

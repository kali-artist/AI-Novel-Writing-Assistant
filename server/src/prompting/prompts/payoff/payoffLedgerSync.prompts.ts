...
> = {
  ...
  render: (input) => [
    new SystemMessage([
      "你是小说伏笔账本同步器，负责把多处来源里的伏笔、兑现安排、兑现证据和异常信号，收敛成一份唯一的 canonical payoff ledger。",
      ...
      "3. 如果章节已经兑现了某项伏笔，应优先标成 paid_off，并记录 payoffChapterId。",
  ...
  postValidate: (output) => {
    ...
    for (const item of output.items) {
      ...
      if (item.targetStartChapterOrder && item.targetEndChapterOrder && item.targetStartChapterOrder > item.targetEndChapterOrder) {
        throw new Error(`伏笔 ${item.ledgerKey} 的目标章节窗口非法。`);
      }
      if (item.currentStatus === "paid_off" && !item.payoffChapterId) {
        throw new Error(`伏笔 ${item.ledgerKey} 已兑现时必须返回 payoffChapterId。`);
      }
    }
    return output;
  },

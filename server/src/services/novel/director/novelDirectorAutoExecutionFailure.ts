...
export function buildSkippableAutoExecutionReviewBlockingReason(
  autoExecution?: Pick<DirectorAutoExecutionState, "nextChapterOrder"> | null,
): string {
  if (typeof autoExecution?.nextChapterOrder === "number") {
    return `当前章因审核阻断而暂停，但这类问题允许跳过当前章继续执行。点击“继续自动执行前 10 章”后，系统会从第 ${autoExecution.nextChapterOrder} 章继续。`;
  }
  return "当前章因审核阻断而暂停，但这类问题允许跳过当前章继续执行。点击“继续自动执行前 10 章”后，系统会从下一章继续。";
}
...
export function buildSkippableAutoExecutionReviewRecoveryHint(
  ...
  if (typeof autoExecution?.nextChapterOrder === "number") {
    return `可直接点击“继续自动执行前 10 章”，系统会跳过当前审核阻断章并从第 ${autoExecution.nextChapterOrder} 章继续；如需修复当前章，再回到章节执行或质量修复处理。`;
  }
  return "可直接点击“继续自动执行前 10 章”，系统会跳过当前审核阻断章并从下一章继续；如需修复当前章，再回到章节执行或质量修复处理。";
}

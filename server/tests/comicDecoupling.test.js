...
test("services/comic 不依赖 drama 服务实现（跨模块复用须经 adaptation 层）", () => {
  ...
  const importRe = /\bfrom\s+['"]([^'"]+)['"]/g;
  // 拦截绝对/相对路径中 services/drama 段，放行 adaptation/ 路径
  const dramaSegmentRe = /(^|\/)drama(\/|$)/;
  const adaptationExemptRe = /(^|\/)adaptation(\/|$)/;
  const violations = [];
  for (const file of files) {
    const src = fs.readFileSync(file, "utf8");
    let match;
    while ((match = importRe.exec(src)) !== null) {
      const spec = match[1].toLowerCase();
      if (dramaSegmentRe.test(spec) && !adaptationExemptRe.test(spec)) {
        violations.push(`${path.relative(COMIC_SRC, file)} → ${match[1]}`);
      }
    }
  }
  ...
  assert.deepEqual(
    violations,
    [],
    `comic 模块禁止直接 import drama 服务实现；共享能力请下沉到 services/adaptation/：\n${violations.join("\n")}`,
  );
});
